		...
		*/
		...
		function mountForm(container, scope) {
			...
			saveBtn.addEventListener("click", async () => {
				...
				} else if (provider === "tavily" && key.length > 0) writes.push(scope.set("tavilyApiKey", key));
				else if (provider === "serper" && key.length > 0) writes.push(scope.set("serperApiKey", key));
				else if (provider === "brave" && key.length > 0) writes.push(scope.set("braveApiKey", key));
				else if (provider === "bing" && key.length > 0) writes.push(scope.set("bingApiKey", key));
