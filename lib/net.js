...
export function isPrivateIp(addr) {
    ...
    if (v4 !== null) {
        ...
        if (a === 198 && (b === 18 || b === 19))
            return true; // 基准测试网段 198.18.0.0/15
        if (a === 192 && b === 88 && c === 99)
            return true; // 6to4 relay anycast
        if (a >= 240)
            return true; // 保留段 240.0.0.0/4（含 255.255.255.255）
        return false;
    }
    ...
    if (lower.startsWith('ff'))
        return true; // 组播 ff00::/8
    if (lower.startsWith('2002:'))
        return true; // 6to4 2002::/16
    if (lower.startsWith('2001:0000:') || lower.startsWith('2001:0:'))
        return true; // Teredo 2001::/32
    if (lower.startsWith('2001:db8:'))
        return true; // 文档用 2001:db8::/32
    if (lower.startsWith('64:ff9b:'))
        return true; // NAT64 well-known prefix 64:ff9b::/96
...
export async function fetchManualRedirects(target, headers, cfg, signal) {
    let current = target;
