...
function dedupeByDomain(sources, limit) {
    if (limit <= 0)
        return sources;
    const seen = new Map();
    const out = [];
...
function sortByRelevance(sources, query) {
        ...
        try {
            ...
            for (const id of targets) {
                ...
                try {
                    ...
                    if (content === undefined && result.content !== undefined)
                        content = result.content;
                    if (!r.cfg.mergeResults && sources.length > 0)
                        break;
                }
                catch (error) {
            ...
            if (r.cfg.cacheEnabled)
                cacheSet(cacheKey, result, r.cfg.cacheTtlMs);
            return result;
        }
        finally {
