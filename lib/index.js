...
import { cacheGetOrCompute, cacheKeyOf, circuitMarkFailure, circuitMarkSuccess, circuitOpen, getCacheStats, getCircuitStates, getSearchStats, recordStat, resetRuntimeState, runWithConcurrency, } from './state.js';
...
async function resolveEngineKeys(r, id) {
    const spec = getEngineSpec(id);
    if (spec === undefined)
        throw new WebError('未知引擎: ' + id, 'WEB_PROVIDER_ERROR');
    const bag = r.cfg;
    const keys = [];
    for (const input of engineInputs(spec)) {
        if (input.envRefKey === undefined || input.envVar === undefined)
            continue; // 非凭据输入
...
function requireKey(providerLabel, value, what = 'API key') {
...
export async function searchSearxng(r, req, signal) {
    ...
    try {
        ...
        if (error instanceof WebError && /\b403\b/.test(error.message)) {
            throw new WebError('SearXNG error: HTTP 403 — 该实例可能未启用 JSON 输出（自建实例需在 settings.yml 的 search.formats 中加入 json），或触发了 bot 检测。建议自建 SearXNG 并配置 searxngBaseURL', 'WEB_PROVIDER_ERROR', { cause: error });
        }
        throw error;
    }
    const raw = Array.isArray(data?.results) ? data.results : [];
    return {
        sources: dedupe(raw.map((item) => toSource(item?.url, item?.title, item?.content, item?.publishedDate))),
    };
}
export async function searchTavily(r, req, signal) {
    ...
    return {
        sources: dedupe(raw.map((item) => toSource(item?.url, item?.title, item?.content, item?.published_date))),
...
export async function searchSerper(r, req, signal) {
    ...
    const body = { q: req.query, num: req.maxResults ?? 8 };
    ...
    return {
        sources: dedupe(raw.map((item) => toSource(item?.link, item?.title, item?.snippet, item?.date))),
...
export async function searchBrave(r, req, signal) {
    ...
    return {
        ...
        sources: dedupe(raw.map((item) => toSource(item?.url, item?.title, item?.description, item?.page_age))),
    };
}
export async function searchBing(r, req, signal) {
    ...
    return {
        sources: dedupe(raw.map((item) => toSource(item?.url, item?.name, item?.snippet, item?.datePublished))),
    };
}
 ...
 *  未知的自定义源 id 默认视为可用。 */
export async function builtinKeyAvailable(ctx, cfg, id) {
    const spec = getEngineSpec(id);
    if (spec === undefined)
        return true;
    const bag = cfg;
...
export const BUILTIN_LABELS = Object.fromEntries(ENGINE_SPECS.map((s) => [s.id, s.label]));
...
export class ProviderRegistry extends Service {
    ...
    register(adapter, internal = false) {
...
export function builtinAdapter(ctx, spec) {
...
export async function buildProviderChain(cfg, registry, ctx) {
    const chain = [];
...
function abortErrorOf(controller) {
 ...
 */
export function syncKeyAvailable(ctx, cfg, id) {
    const spec = getEngineSpec(id);
    if (spec === undefined)
        return true; // 自定义源：交给 adapter.available
    const bag = cfg;
    ...
    try {
        ...
        hasCredentialsService = false;
    }
    for (const input of engineInputs(spec)) {
        if (input.envRefKey === undefined || input.envVar === undefined)
            continue; // 非凭据输入
        if ((bag[input.configKey] ?? '').length > 0)
            continue; // 字面量已配置
        let ambient = '';
        try {
            ambient = launchEnvironmentOf(ctx).get(bag[input.envRefKey] || input.envVar)?.value ?? '';
        }
        catch {
            ambient = '';
        }
...
export class ThirdPartySearchProvider {
    ...
    available() {
        try {
            ...
            if (registry === undefined || !registry.sources.has(r.cfg.provider))
                return false;
    ...
    async search(request, signal) {
        ...
        try {
            const compute = async () => {
                const chain = await buildProviderChain(r.cfg, registry, r.ctx);
                ...
                if (r.cfg.relevanceSort)
                    processed = sortByRelevance(processed, request.query);
...
export function cfgFromTestBody(cfg, body) {
    const next = { ...cfg };
...
export class LocalFetchProvider {
    ...
    async fetch(request, signal) {
        ...
        try {
            ...
            if (rawText.length > parseLimit)
                rawText = rawText.slice(0, parseLimit);
