...
function toSource(url, title, snippet, published) {
...
function activeEndpointsOf(cfg) {
    const bag = cfg;
...
export function isPrivateIp(addr) {
    ...
    if (v4 !== null) {
        ...
        if (a > 255 || b > 255 || c > 255 || d > 255)
            return true; // 非法段按不可信处理
        if (a === 0 || a === 10 || a === 127)
            return true; // this-network / 私网 / 环回
        if (a === 172 && b >= 16 && b <= 31)
            return true;
        if (a === 192 && b === 168)
            return true;
        if (a === 169 && b === 254)
            return true; // link-local（云元数据）
        if (a === 100 && b >= 64 && b <= 127)
            return true; // CGNAT
        return false;
    }
...
export function isPrivateName(host) {
...
export async function assertPublicUrl(url, cfg) {
...
async function fetchManualRedirects(target, headers, cfg, signal) {
    let current = target;
